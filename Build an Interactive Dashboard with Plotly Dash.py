...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names ='Launch Site', 
        title ='Number of Successes for All Launch Sites')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        df1 = filtered_df.groupby(['Launch Site','class']).size().reset_index(name = 'class count')
        fig = px.pie(df1, values = 'class count', names = 'class', title = f"Total Successful Launches for Site {entered_site}")
        return fig
...
def get_scatter_plot(entered_site, slider_value):
    ...
    if entered_site == 'ALL':
        ...
        title = 'Relationship Btw Payload and Success for All Launch Sites')
        return fig
    else:
        # return the outcomes scatter plot for a selected site
        filtered_df2 = filtered_df2[filtered_df2['Launch Site'] == entered_site]
        fig = px.scatter(filtered_df2, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category', title = ('Correlation Between Payload and Success for site {entered_site}'))
        return fig
